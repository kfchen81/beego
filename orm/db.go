...
func addComment(q dbQuerier, query string) string {
	...
	case *dbQueryTracable:
		comment := q.(*dbQueryTracable).comment
		if len(comment) > 0 && !strings.Contains(query, comment) {
			query = fmt.Sprintf("%s %s", q.(*dbQueryTracable).comment, query)
		}
	default:
	}
...
func addForceIndex(qs *querySet, query string) string {
	if qs.index == "" {
		return query
	}

	items := strings.Split(query, "WHERE")
	items[0] = fmt.Sprintf("%s FORCE INDEX (%s) WHERE", items[0], qs.index)
